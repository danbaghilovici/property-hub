...
resource "aws_iam_role_policy_attachment" "property_hub_backend_iam_role_policy_attachment-dynamo" {
  policy_arn = var.aws_iam_policy_arn_dynamo
  role       = aws_iam_role.property_hub_backend_iam_role.name
}
...
resource "aws_lambda_function" "property_hub_backend_lambda" {
  ...
  timeout          = 30
  environment { variables = { ENV : terraform.workspace } }
  snap_start {
